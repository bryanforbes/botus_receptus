...
def test_setup_logging(mocker, mock_get_logger, mock_formatter, mock_file_handler, mock_stream_handler):
    parser = ConfigParser()
    parser.read_string('''
[bot]
bot_name = botty
...
log_level = info''')
    ...
    with setup_logging(parser):
        ...
        mock_file_handler.assert_called_with(filename='botty.log', encoding='utf-8', mode='a')
        mock_file_handler.return_value.setFormatter.assert_called_with(mock_formatter.return_value)
        mock_get_logger.return_value.addHandler.assert_called_with(mock_file_handler.return_value)
        mock_stream_handler.assert_not_called()


def test_setup_logging_console(mocker, mock_get_logger, mock_formatter, mock_stream_handler):
    parser = ConfigParser()
    parser.read_string('''
[bot]
bot_name = botty
...
log_level = info''')
    ...
    with setup_logging(parser):
        ...
        mock_get_logger.return_value.addHandler.assert_called_with(mock_stream_handler.return_value)


def test_setup_logging_loggers(mocker, mock_get_logger, mock_formatter, mock_file_handler, mock_stream_handler):
    parser = ConfigParser()
    parser.read_string('''
[bot]
bot_name = botty
